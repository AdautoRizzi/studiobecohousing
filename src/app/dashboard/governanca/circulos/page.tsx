    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <h1 className="text-2xl font-bold text-primary-900">Círculos e Domínios</h1>
                <button className="bg-primary-600 text-white px-4 py-2 rounded-lg text-sm font-medium">Novo Círculo</button>
            </div>
                ...
                {circles.map((c: any) => (
                    <div key={c.id} className="bg-white p-6 rounded-xl border border-secondary-200 shadow-sm relative">
                        <div className="absolute top-4 right-4 bg-secondary-100 text-secondary-800 text-xs px-2 py-1 rounded-full font-bold">
                            {c.parentCircleId ? 'Sub-círculo' : 'Círculo Geral'}
                        </div>
                        <h2 className="text-xl font-bold text-primary-900">{c.name}</h2>
                        <div className="mt-4 space-y-2">
                            <p className="text-sm text-gray-700"><strong>Domínio (Autoridade):</strong> {c.domain}</p>
                            <p className="text-sm text-gray-700"><strong>Propósito (Aim):</strong> {c.aim}</p>
                        </div>
