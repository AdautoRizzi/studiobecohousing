...
export default async function CRMPage({ searchParams }: { searchParams: Promise<{ tab?: string, q?: string, sort?: string, order?: string }> }) {
    ...
    const getStatusColor = (status: string) => {
        switch(status) {
            ...
            default: return 'bg-slate-800 text-slate-100 border-slate-800';
        }
    };
    ...
    return (
        <div className="w-full max-w-full min-w-0">
            <div className="flex justify-between items-end mb-8">
                <div>
                    ...
                    <div className="flex gap-4 mt-2">
                        <p className="text-slate-400">Acompanhe todos os interessados no projeto Studio Be.</p>
                ...
                <div className="flex gap-4 items-end">
                    <Link href="/admin/crm/importar" className="bg-slate-800 text-gray-700 px-6 py-2.5 rounded-xl text-sm font-bold hover:bg-gray-200 transition-all flex items-center gap-2 border border-slate-800">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                {/* Abas de Navegação */}
                <div className="flex space-x-2 bg-[#0f172a] p-2 rounded-2xl shadow-sm border border-slate-800 overflow-x-auto w-full md:w-auto">
                    <Link href={`/admin/crm?tab=leads${searchQuery ? '&q='+searchQuery : ''}`} scroll={false} className={`px-6 py-2.5 rounded-xl text-sm font-bold transition-all whitespace-nowrap ${currentTab === 'leads' ? 'bg-secondary-50 text-secondary-600 shadow-sm border border-secondary-100' : 'text-slate-400 hover:bg-[#020617]'}`}>
                        🚀 Leads Site ({sortedLeads.filter(l => getCleanCategory(l.categoria) === 'Lead Site').length})
                    </Link>
                    <Link href={`/admin/crm?tab=pesquisa${searchQuery ? '&q='+searchQuery : ''}`} scroll={false} className={`px-6 py-2.5 rounded-xl text-sm font-bold transition-all whitespace-nowrap ${currentTab === 'pesquisa' ? 'bg-primary-50 text-primary-700 shadow-sm border border-primary-100' : 'text-slate-400 hover:bg-[#020617]'}`}>
                        📊 Pesquisa Antiga ({sortedLeads.filter(l => getCleanCategory(l.categoria) === 'Pesquisa Antiga').length})
                    </Link>
                    <Link href={`/admin/crm?tab=stakeholders${searchQuery ? '&q='+searchQuery : ''}`} scroll={false} className={`px-6 py-2.5 rounded-xl text-sm font-bold transition-all whitespace-nowrap ${currentTab === 'stakeholders' ? 'bg-gray-800 text-white shadow-sm' : 'text-slate-400 hover:bg-[#020617]'}`}>
                ...
                <div className="w-full md:w-auto">
                    <Suspense fallback={<div className="w-full md:w-64 h-11 bg-slate-800 animate-pulse rounded-xl mb-6"></div>}>
                        <SearchInput />
                    </Suspense>
                </div>
            </div>

            <div className="bg-[#0f172a] rounded-2xl shadow-sm border border-slate-800 overflow-x-auto w-full">
                <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-[#020617]">
                        <tr>
                            <th className="px-6 py-4 text-left text-xs font-bold uppercase tracking-wider">
                                <Link href={getSortUrl('contato')} scroll={false} className="flex items-center text-slate-400 hover:text-slate-50 transition-colors">
                                    Contato {renderSortIcon('contato')}
                                </Link>
                            </th>
                            <th className="px-6 py-4 text-left text-xs font-bold uppercase tracking-wider">
                                <Link href={getSortUrl('idade')} scroll={false} className="flex items-center text-slate-400 hover:text-slate-50 transition-colors">
                                    Idade / Perfil {renderSortIcon('idade')}
                                </Link>
                            </th>
                            <th className="px-6 py-4 text-left text-xs font-bold uppercase tracking-wider">
                                <Link href={getSortUrl('local')} scroll={false} className="flex items-center text-slate-400 hover:text-slate-50 transition-colors">
                                    Local / Desejo {renderSortIcon('local')}
                                </Link>
                            </th>
                            <th className="px-6 py-4 text-left text-xs font-bold uppercase tracking-wider">
                                <Link href={getSortUrl('cadastro')} scroll={false} className="flex items-center text-slate-400 hover:text-slate-50 transition-colors">
                                    Cadastro {renderSortIcon('cadastro')}
                                </Link>
                            </th>
                            <th className="px-6 py-4 text-left text-xs font-bold uppercase tracking-wider">
                                <Link href={getSortUrl('status')} scroll={false} className="flex items-center text-slate-400 hover:text-slate-50 transition-colors">
                    ...
                    <tbody className="bg-[#0f172a] divide-y divide-gray-200">
                        {filteredLeads.length === 0 ? (
                            ...
                            filteredLeads.map(lead => (
                                <tr key={lead.id} id={`lead-${lead.id}`} className="hover:bg-[#020617] transition-colors scroll-mt-24">
                                    <td className="px-6 py-4">
                                        <div className="font-bold text-slate-50">{lead.nome}</div>
                                        <div className="text-sm text-slate-400">{lead.email}</div>
                                        <div className="text-sm text-slate-400">{lead.telefone || 'Sem telefone'}</div>
                                    </td>
                                    <td className="px-6 py-4">
                                        <div className="text-sm text-slate-50">{lead.idade}</div>
                                        <div className="text-sm text-slate-400">{lead.profissao}</div>
                                    </td>
                                    <td className="px-6 py-4">
                                        <div className="text-sm text-slate-50">Mora em: {lead.moradiaAtual}</div>
